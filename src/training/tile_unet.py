...
def parse_args():
    ...
    parser.add_argument('--bands', '-b', type=str, default='0', required=False,
                        help='sting with bands seperated by commas. defaults to 0 for the panchromatic file and to 2,4,5 for multispectral')
    ...
    parser.add_argument('--geotiff', '-g', type=int, default=1, required=False,
...
def write_tile(scn, bands, patch_size, output_folder, offset, geotiff,n):
    # read window
    ...
    filename = "%s/%d.tif" % (output_folder,n )

    if (geotiff and len(bands)==1):
...
def tile_raster(input_image, output_folder, bands, patch_size, stride=1, geotiff=0):
    ...
    for offset in offsets:
        n += 1
        write_tile(src_scn, bands, patch_size, output_folder, offset, geotiff,n)
